...
# Keep the parsed index in memory between ToolNode calls.  The cache is
# invalidated after writes and also notices external changes through the file
# stat signature, so a long-running Streamlit process does not repeatedly parse
# the same JSON file while still seeing edits made by another process.
_INDEX_CACHE: dict[Path, tuple[tuple[int, int] | None, list[dict[str, Any]]]] = {}
...
_ERROR_PREFIX_RE = re.compile(
    r"^Error\s*(?:[:：]|(?:retrieving|fetching|loading|reading|requesting|"
    r"calling|connecting|getting|accessing)\b)",
    re.IGNORECASE,
)
...
def _index_file_signature(path: Path) -> tuple[int, int] | None:
    try:
        stat = path.stat()
    except OSError:
        return None
...
def _load_index() -> list[dict[str, Any]]:
    ...
    signature = _index_file_signature(path)
    ...
    _INDEX_CACHE[path] = (signature, entries)
    return entries
...
def _save_index(entries: list[dict[str, Any]]) -> None:
    ...
    # the index lock.
    _INDEX_CACHE[path] = (_index_file_signature(path), list(entries))
...
def _diagnose_result(content: Any, *, status: str = "success") -> dict[str, Any] | None:
    ...
    stripped = text.strip()
    # A successful tool may return a news article whose title starts with
    # ``Error``.  Treat that prefix as a failure only for short, structured
    # error responses; actual ToolMessage errors are covered by ``status`` and
    # the more specific patterns below.
    error_prefix = bool(
        stripped
        and len(stripped) <= 200
        and _ERROR_PREFIX_RE.search(stripped)
    )
    failed = (
        status == "error"
        or error_prefix
        or any(pattern.search(text) for pattern in _FAILURE_PATTERNS)
    )
